...
use core::{str, sync::atomic::{AtomicBool, Ordering}};
use defmt::*;
...
async fn main(spawner: Spawner) {
    ...
    let sd: &mut Softdevice = Softdevice::enable(&sd_config);
    ...
    let keyboard_pins = keyboard::KeyboardPins::new(
        p.P0_29, p.P0_02, p.P1_15, p.P1_13, p.P1_11, p.P1_04, p.P1_06, p.P0_09, p.P0_10,
    );

    // spawner.spawn(lusb::usb_task(p.USBD)).unwrap();
    // spawner.spawn(sble::ble_task(&sd)).unwrap();
    spawner
        .spawn(keyboard::keyboard_task(keyboard_pins))
        .unwrap();
    // spawner.spawn(sble::ble_task(sd)).unwrap();
